...
int has_cycle(node *head)
{
  ...
  node *ptr1, *ptr2; //Declare two pointers
  ptr1 = ptr2 = head; //Points them at head

  //This loop will decide if an non-length-zero list has a cycle or not.
  while(1){
    if (ptr1->next == NULL){return 0;} //There should never be a null. Checks before assignment.
    ptr1 = ptr1->next; //Increments pointer by one after assuring it has a next.

    if (ptr2->next == NULL || ptr2->next->next == NULL){return 0;} //There should never be a null. Checks both nexts before assignment. 
    ptr2 = ptr2->next->next; //Increments pointer by two after assuring next has a next.

    if (ptr1 == ptr2 || ptr1 == ptr2->next){return 1;} //Either condition is met. 
  }
}
